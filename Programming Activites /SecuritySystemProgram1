### The prompt that generated the code below
# We have a Arduino nano esp32.
# Program in micro python.

# Refer to the documentation for Arduino nano esp32.

# we have a light sensor connected to A7 (D24 or GPI014). This INPUT is analog.
# we have a MOMENTARY CONTACT SWITCH connected to d12 (GPI047). This INPUT is digital and will only be true while the button is PRESSED otherwise FALSE.
# we have green light connected to d2 (GPI05). This OUTPUT is digital.
# we have a red light connected to d3 (GPI06). This OUTPUT is digital.

# Write a program that first reads the light sensor and compares it to a external variable that we set. It should be checking this light sensor the entire time the system is POWERED ON. If the light sensor outputs a number less than the external variable, the system should be enabled. Otherwise, the system is disabled and NOTHING should happen. Whether the system is enabled or disabled, the light sensor should always be checked to see if its value reported is lower than the external variable. When the system is ENABLED, the GREEN LED on the Arduino nano esp32 should turn on. Now, when the MOMENTARY CONTACT SWITCH is pressed, it should set a external Boolean value to TRUE. When this external Boolean value is TRUE, the red light should flash on and off continuously. The flashing interval should be determined by a external variable. When the system is DISABLED, this Boolean value will be reset to FALSE. All the external variables should be placed somewhat on the top of the program for easy access and editing. The system should also report it's inputs and outputs every second to the console.
